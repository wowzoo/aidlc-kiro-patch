...
function parseActiveDirectiveMarker(parsed: unknown): ActiveDirectiveMarker | null {
  ...
  if (
    ...
    ("steering_bundle" in parsed &&
      !/^sha256:[0-9a-f]{64}$/.test(String(parsed.steering_bundle ?? ""))) ||
...
export function writeActiveDirectiveMarker(
  ...
  const result = transactActiveDirective(projectDir, (current, target) => {
    ...
    // LOCAL DELTA (aidlc-*-patch): monotonic republication. Stock preserves the
    // authority revision only on the swarm-shaped path (both kinds load-steering
    // /invoke-swarm AND `units` non-empty), so a solo per-unit directive never
    // qualified: every publication rotated the revision and, because
    // shouldResetRuntime is its negation, `rmSync`d the Plan Approval runtime
    // (challenge, response and receipts). Re-publishing the SAME directive is not
    // a new issuance, so it keeps both. A genuinely different one still rotates.
    const republishesSameDirective =
      marker.stage === "code-generation" &&
      base.stage === "code-generation" &&
      base.kind === marker.kind &&
      (base.unit ?? null) === (marker.unit ?? null) &&
      base.steering_bundle !== undefined &&
      base.steering_bundle === marker.steering_bundle &&
      !rotateCodeGenerationFloor &&
      base.code_generation_source_sha256 !== undefined;
    const preserveCodeGenerationAuthority =
      republishesSameDirective ||
      (marker.stage === "code-generation" &&
        base.stage === "code-generation" &&
        baseSwarmPlanning &&
        nextSwarmPlanning &&
    ...
    const next: ActiveDirectiveMarker = {
      ...
      ...(marker.steering_bundle
        ? { steering_bundle: marker.steering_bundle }
        : { steering_bundle: undefined }),
      delivery: "issued",
...
export function advanceContinuationCursor(
  ...
  const result = transactActiveDirectiveTarget(snapshot.target, (current, target) => {
    ...
    const next: ActiveDirectiveMarker = {
      ...
      ...(token ? { continue_token: token, continue_token_sha256: stateContentSha256(token) } : { continue_token: undefined, continue_token_sha256: undefined }),
      ...(successor.steering_bundle
        ? { steering_bundle: successor.steering_bundle }
        : { steering_bundle: undefined }),
      delivery: "issued",
...
function currentUnitLifecycleRows(
  ...
  // Sort before use: this function hands these rows to
  // latestMainWorkflowStageRunFloorForProject, which treats caller-supplied rows
  // as pre-sorted. readAuditShardEvents returns them in shard order, so with more
  // than one shard the floor computed here would disagree with the floor a writer
  // recorded, and every receipt row would be discarded below.
  const sourceRows = [...(auditRows ?? readAuditShardEvents(projectDir))].sort(
    (a, b) => {
      if (a.timestamp !== b.timestamp) {
        return a.timestamp < b.timestamp ? -1 : 1;
      }
