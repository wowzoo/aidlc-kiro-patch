...
| `WORKTREE_MERGED` | Bolt's worktree merged back to its target branch when the `merge` verb is run | Timestamp, Bolt slug, Worktree path, Target branch, Strategy | `tools/aidlc-worktree.ts` (`merge`) |
...
| `STATE_MERGED` | Worktree's state merged back to the main state when `aidlc-bolt complete --merge` is run | Timestamp, Bolt slug, Worktree path, Source state hash, Target state hash, Conflict resolution | `tools/aidlc-state.ts` (`merge`) |
...
| `AUDIT_MERGED` | Worktree's audit entries appended to main audit when `aidlc-bolt complete --merge` is run; per-Bolt entry order preserved, cross-Bolt order reflects merge-completion order | Timestamp, Bolt slug, Entries Merged, Source Audit Hash, Fork Boundary, Fork Timestamp | `tools/aidlc-audit.ts` (`audit-merge`) |
