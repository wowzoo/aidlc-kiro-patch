...
Neither Kiro surface has a structured-question tool, so every structured question
renders as **numbered prose options in chat**, and the user answers with a number
(or free text). Render the spec like this:
...
path is the compatibility contract across both Kiro surfaces and their versions.
