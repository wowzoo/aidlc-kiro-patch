...
function extractResponseText(value: unknown): string {
  if (typeof value === "string") {
    ...
    try {
      ...
      // A bare JSON scalar carries no richer text than the literal the human
      // typed, and parsing it discards the reply: "1" becomes the number 1,
      // which is neither string nor object, so the walk below returns "". The
      // engine offers that very shortcut (offeredPlanApprovalChoice accepts
      // "1"/"2"), so the parse must not consume it. A JSON-quoted string is
      // still unwrapped, and structured payloads still get walked.
      if (typeof parsed === "string") return extractResponseText(parsed);
